...
int keys_p2[5] = { 0x43, 0x42, 0x3b, 0x4b, 0x33}; // IKJL -> H (Default)
#define ONLY_JOYKEYS(a) a
#else
#define ONLY_JOYKEYS(a)
#endif

static struct menu_entry *menu;
static int menu_visible=0;

#define SW_DBLWIN			0x20
...
void Menu_Hide()
{
	...
	while(TestKey(HOTKEY) || TestKey(KEY_ENTER) ONLY_JOYKEYS(|| TestKey(keys_p1[4])) )
		HandlePS2RawCodes();
...
int Menu_Run()
{
	...
	if((TestKey(KEY_UPARROW)&2) ONLY_JOYKEYS(|| (TestKey(keys_p1[0])&2)) )
	{
		if(currentrow)
			--currentrow;
	...
	if((TestKey(KEY_DOWNARROW)&2) ONLY_JOYKEYS(|| (TestKey(keys_p1[1])&2)))
	{
